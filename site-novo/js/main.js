(function () {
  ...
  /* ---------- indicador do carrossel de páginas do PDF ----------
     O card que ocupa mais de 60% da trilha é o "atual". */
  var track = document.querySelector('.provas-track');
  ...
  var revealObs = new IntersectionObserver(
    function (entries) {
      entries.forEach(function (en) {
        ...
        if (el.classList.contains('provas-track')) {
  ...
  pending.forEach(function (el) {
    ...
    var t = el.closest('.provas-track');
    if (t) {
      if (tracked.indexOf(t) === -1) {
        tracked.push(t);
        revealObs.observe(t);
      }
      return;
